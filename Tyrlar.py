...
def main_menu_kb(lang, chat_id):
    ...
    kb.add(types.KeyboardButton(L(lang, "📋 Ýüklerim", "📋 Мои грузы")))
    ...
    kb.add(types.KeyboardButton(L(lang, "🚚 Boş maşyn gözle", "🚚 Искать свободную машину")))
    kb.add(types.KeyboardButton(L(lang, "⭐ VIP", "⭐ VIP")))
    kb.add(types.KeyboardButton(L(lang, "👤 Profil", "👤 Профиль")))
...
def cb_adm_loads(call):
    ...
    for d in docs:
        ...
        vip_mark = " ⭐" if d.get("vip") else ""
        lines.append(f"{status_icon} {d['from_loc']} - {d['to_loc']} | {d['price']}${vip_mark} | {d.get('status')}")
...
def cb_adm_drivers(call):
    ...
    for d in docs:
        vip_mark = " ⭐" if d.get("vip") else ""
        lines.append(f"@{d.get('username','-')} | {d['from_loc']} - {d['to_loc']} | {d.get('model','-')} | {d.get('ton')}t{vip_mark}")
